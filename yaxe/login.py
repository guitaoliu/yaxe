...
from yaxe.config import config
from yaxe.utils import encrypt_passward, get_timestamp
...
def ehall_login() -> requests.Session:
    ...
    session.cookies.set("cur_appId_", "GRt5IN2Ni3M=")
    resp = session.get(
    ...
    resp = session.get(
        ...
        data={"userName": config.net_id, "_": get_timestamp()},
    )
